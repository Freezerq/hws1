...
function Clock() {
    ...
    const start = () => {
        ...
        console.log("Click")
        let timerrr = setInterval(() => {
            setDate(new Date(Date.now()))
        }, 1000)
        console.log(timerrr)
        setTimerId(Number(timerrr))
    ...
    let formatterDayMonthYear = new Intl.DateTimeFormat("ru");

    const stringTime = formatter.format(date) || <br/> // часы24:минуты:секунды (01:02:03)/(23:02:03)/(24:00:00)/(00:00:01) // пишут студенты
    const stringDate = formatterDayMonthYear.format(date) || <br/> // день.месяц.год (01.02.2022) // пишут студенты, варианты 01.02.0123/01.02.-123/01.02.12345 не рассматриваем
